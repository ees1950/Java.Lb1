    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numberOfObjects; i++) {
            if (random.nextInt(2) == 0) {
                shoesArrayList.add(new Nike(random.nextInt(10000), random.nextInt(1000),  random.nextInt(1000),
                        random.nextInt(10000),  random.nextInt(20000), getRandomColor(), getRandomModelName()));
            } else {
                shoesArrayList.add(
                        new Vans (random.nextInt(1000), random.nextInt(100), random.nextInt(500), getRandomColor(),
